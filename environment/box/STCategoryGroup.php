...
class STCategoryGroup extends TableTag
{
	var $sMethod= "get";
	var $categoryName;
	var $aoTable= array();
	...
	{		
		...
		if($nCategorys)
		{
			Tag::echoDebug("searchBox", "create ".$nCategorys." categorys in ".$this->categoryName);
			$countCategorys= 1;	
			$nCategorys= count($this->aCategorys);
			...
			//foreach($this->aCategorys as $group)	
			{
				...
				Tag::echoDebug("searchBox", "  found inside ".$count." categorys ");
				if(	$count==1
					and
					!$mainCategory)
				{
		...
		{
			Tag::echoDebug("searchBox", "create category ".$this->categoryName);
			$tr= new RowTag();
			$td= new ColumnTag(TD);
			$td->valign("top");
			$td->add($mainCategory);
	...
	/*protected*/function getThisCategoryTags($catCount)
	{
    	...
    	if(isset($GetPost["stget"]["searchbox"][$this->categoryName]))
	...
	function check($text)
	{
		...
		if($params["stget"]["searchbox"])
			return;// if in the params the checkfields defined, take this one		
...
/*		// f�r identification �ber get oder post
	...
	function createWhere(&$oTable)
	{
		...
		{//st_print_r($checkBox);					
			...
			{
			 	Tag::echoDebug("searchBox", "category '".$checkBox["name"]."' is checked!");
				$newWhere->andWhere($checkBox["then"]);
			}else
			{
			  Tag::echoDebug("searchBox", "category '".$checkBox["name"]."' is not checked!");
	...
	function defineCaseSensitiveButton($break= false, $name= "Gro�/Kleinschreibung beachten")
	{
	...
	function defineWholeWordsButton($break= false, $name= "Nur ganze W�rter")
	{
	...
	function defineMatchButtons($break= false, $sensitive= "Gro�/Kleinschreibung beachten&#160;&#160;", $words= "Nur ganze W�rter")
	{
	...
	function defineSearchButtons($break= false,	$and= "und", $or= "oder&#160;&#160;",
												$sensitive= "Gro�/Kleinschreibung beachten&#160;&#160;", $words= "Nur ganze W�rter")
	{
