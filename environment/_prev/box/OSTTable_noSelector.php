...
class OSTTable extends OSTBaseTableBox
{
		...
		var $deleteStatement;
		var $showTypes;
		var	$oGet; // Objekt f�r vorhandene Parameter (GetHtml)
		var $nShowFirstRow= 0;
		...
		function OSTTable(&$container, $class= "OSTTable", $logTime= false)
		{
			...
			if($this->log)
			{
				...
				$debug= $user->isDebug();
				$user->debug(true);
				$user->LOG("start OSTTable->Konstructor", 0);
				$user->debug($debug);
			}
			...
			$this->msg->setMessageContent("NO_CHECKONDB", "vor dem display-Aufruf muss der Funktion checkOnDb() ein DB-Statement �bergeben werden");
			$this->msg->setMessageContent("NO_CHANGING", "beim Checkbox-Abgleich wurde nichts ver�ndert");			
			...
			$this->msg->setMessageContent("NNTABLEINSERT_MUCH@2", "f�r den neuen Insert der n zu n Table sind zu viele Ergebnisse f�r die Spalte \"@\", im where-statement vorhanden");
			$this->msg->setMessageContent("NNTABLEINSERT_FAULT@", "f�r den neuen Insert der n zu n Table ist die Spalte \"@\" nicht gesetzt");
			$this->msg->setMessageContent("NOPK_FORDBCHANGE@", "f�r die �nderung in der Datenbank ist der prim�rere Schl�ssel \"@\" nicht im Ergebnis.");
	...
	function createStatement()
	{
		$HTTP_GET_VARS= new GetHtml();
  		...
  		{// es wurde als Tabelle ein STDbTable angegeben	
			...
			{
				$callbackClass= new OSTCallbackClass($this->db, $this->sqlResult);
			...
			//					zb. f�r MaxRowSelect wenn nicht beim ersten eintrag
			//					in die Tabelle gesprungen wird
			//					mach ein order by auf den Pk
			if(!count($oTable->asOrder))
				$oTable->orderBy($oTable->getPkColumnName());
			// �berpr�fe ob alle get-Columns aus der Db selektiert werden
			...
			{	
				...
				$limit= $HTTP_GET_VARS["stget"][$tableName]; 
				if($limit)
				{
			...
			}elseif(count($oTable->dateIndex))
			{
    			...
    			{	
					...
					echo "createStatement f�r STWEEK noch nicht ausprogrammiert";exit;
				...
				if($toDateColumn)
				{
					$where=  new OSTDbWhere($fromDateColumn."<'".$todate."'");
					$where->andWhere($toDateColumn.">='".$fromdate."'");
				}else
				{
					$where= new OSTDbWhere($fromDateColumn.">='".$fromdate."'");
			...
			//st_print_r($oTable->show,2);
			//st_print_r($oTable->showTypes,2);
  			$this->statement= $tableDb->getStatement($oTable);
	...
	function makeResult()
	{
		...
		if(!$bDone)
		{
			if(	$this->statement
  			...
  			{				
  				...
  				$this->SqlResult= $this->db->fetch_array($this->statement, MYSQL_ASSOC, $this->getOnError("SQL"));
		...
		/*private*/function getHeadRowAddress($fromColumn)
		{
			...
			if(!is_array($this->aSorts))
			{
				...
				$aSorts= $HTTP_GET_VARS["stget"]["sort"];
				$this->aSorts= array();
				if(is_array($aSorts))
				{
					foreach($aSorts as $column=>$value)
					{
						preg_match("/([^_]+)_([^_]+)/", $value, $nValue);
						$arr= array();
						$arr["column"]= $nValue[1];
						$arr["sort"]= $nValue[2];
						$this->aSorts[]= $arr;
				...
				// toDo: 2005/03/15 alex: Sortierung
				// print_r($this->aSorts);
			}	
			if($this->aSorts[0]["column"]==$fromColumn)
			{
				$aSorts= $this->aSorts;
				if($aSorts[0]["sort"]=="ASC")
					$aSorts[0]["sort"]= "DESC";
				else
					$aSorts[0]["sort"]= "ASC";
			}else
			...
			foreach($aSorts as $value)
			{
				if(	$count!=0
					and
					$value["column"]==$fromColumn	)
				{
					// die Column braucht nicht zweimal im Sortier-Algoritmus sein
				}else
				{
					$oGet->getParamString(STUPDATE, "stget[sort][$count]=".$value["column"]."_".$value["sort"]);
					$count++;
				}
			}
		...
		{			
			...
			$tableName= $oTable->getName();
			if($HTTP_GET_VARS["stget"]["firstrow"][$tableName])
			{
			 ...
			 *****************************************************************/			 
			 ...
			 {	// alex 15/06/2005:	wenn keine Tabelle existiert,
				...
				if(!$this->bGetedSearchboxResult)
				{
					...
					$cTab->modifyForeignKey($oTable->modify());
					$cTab->clearSelects();
    				if($oTable->oSearchBox)
				...
				{
					$TabResult= &$oTable->oSearchBox->getResult_array($tableName);
					$nMaxTableRows= count($TabResult);
				}
			 ...
			 { 
					...
					$needAlwaysIndex										)
				{
					...
					if($this->limitColumnName)
					{
			 		...
			 		$firstRow= $script.$get->getParamString(STUPDATE, $param.$firstRow)."'";
			 		$backRow= $script.$get->getParamString(STUPDATE, $param.$backRow)."'";
			 		$nextRow= $script.$get->getParamString(STUPDATE, $param.$nextRow)."'";
			 		$lastRow= $script.$get->getParamString(STUPDATE, $param.$lastRow)."'";
        ...
        foreach($Rows as $rowKey=>$rowArray)
        {
			...
			foreach($rowArray as $columnKey=>$columnValue)
          	{
				...
				// $key is always the column
				// $rowKey and $columnKey chances by HORIZONTAL/VERTICAL
				// $created Row is alwas the row
				// and $createdColumn always the column
					/*echo "key:$key<br />";
				...
				//echo "columnValue:";st_print_r($columnValue);echo "<br />";
						...
						$this->bSetLinkByNull						)	)
          		{
						...
						!$this->address[$createdColumn]	)
          			{
							...
							if($this->setParams["onActivate"][$columnKey])
							{
								foreach($this->setParams["onActivate"][$columnKey] as $work)
								{
									$get->getParamString($work["do"], $work["param"]);
								}
							}
							if(preg_match("/^container_/", $extraField))
			...
			//echo "file:".__file__." line:".__line__."<br />";st_print_r($make);echo "<br />";					
							...
							}else	
							{
								...
								$get->getParamString(STUPDATE, "stget[".$extraField."][".$asParam."]=".$columnValue);
							}
							$table= &$this->getTable();
							//echo "value is $sValue<br />";
							//echo "represent is $represent<br />";
							if(is_array($table->linkParams[$createdColumn]))
							{
