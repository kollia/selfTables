...
class OSTUserSession extends STSession
{
	...
	function getDynamicClusters($table)
	{
    	...
    	if(!is_array($clusters))
  		{//echo __file__.__line__."<br />";
			...
			$selector= new OSTDbSelector($table);
			$selector->execute();
	...
	{// hole alle Cluster,
	 ...
	 // aus der Datenbank
		...
		$clusterSelector= new OSTDbSelector($oCluster, STSQL_ASSOC);
		...
		//$projectWhere->orWhere();
		$projectCluster= new OSTDbSelector($oProject, STSQL_ASSOC);
	...
	function selectGroupID($groupname)
	{
		...
		$selector= new OSTDbSelector($group);
	...
	function acceptUser($user, $password= null)
	{
		...
		$userSelector= new OSTDbSelector($userTable);
	...
	function getPartitionID($partitionName)
	{
		...
		$selector= new OSTDbSelector($oPartition);
		$selector->clearSelects();
